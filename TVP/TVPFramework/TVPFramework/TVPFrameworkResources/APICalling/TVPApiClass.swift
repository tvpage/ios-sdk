...
public class TVPApiClass: NSObject {
    //MARK: - Retrieve videos from a Product.
    public class func productVideoList(strLoginID:String,strProductID:String, completion : ((NSArray,String)->())?){
        let strURL = ("\(Constants.baseURL)products/\(strProductID)/videos?X-login-id=\(strLoginID)")
        let headers = [
